...
from utils.crawl_coupang import *
from utils.getdata import get_coupang_info, get_collection
...
def get_final_price(df):
    
    if df.y_user_price == -1:
        return int(df.n_user_price)

    elif df.n_user_price == -1:
        return int(df.y_user_price)

    elif (df.n_user_price != -1) and (df.y_user_price != -1):
        return int(df.y_user_price)
...
#     # for airflow test 
#     # insert_price_info(prices_collection, insert_prices, final_result['price_id'])
    

def save_result():
    insert_prices, price_id = coupang_info_manual()
    now = time
    test_df = pd.DataFrame(columns = ["id", "price", "time"], index = range(len(insert_prices)))
    test_df.time = now.strftime('%Y-%m-%d %H:%M:%S')
    test_df.price = insert_prices 
    test_df.id = price_id
    test_df.to_excel('/opt/airflow/dags/output/coupang_extracted.xlsx')
...
with DAG(dag_id = 'update_coupang',
          default_args = args,
    ...
    # t1 = PythonOperator(task_id = 'update_coupang',
    #                     provide_context = True,
    #                     python_callable = coupang_info_manual,
    #                     dag = dag)
    
    t2 = PythonOperator(task_id = 'save_result',
                        provide_context = True,
